...
SymbolType SymbolContextBase::getSymbolType(const shared_ptr<Symbol> name) const throw() {
	return this->resolveSymbolEntry(name)->type;
}

void SymbolContextBase::addType(shared_ptr<Symbol> name, shared_ptr<Type> type) throw() {
...
void SymbolContextBase::addAlias(shared_ptr<Symbol> name, shared_ptr<SymbolRef> symRef) throw() {
	if (!this->assertDoesntExist(name)) {
		// don't add the new type; it just becomes an invalid type after this if used improperly.
		return;
	}
...
shared_ptr<Type> SymbolContextBase::resolveType(shared_ptr<Symbol> name) const throw(runtime_error) {
...
shared_ptr<SymbolEntryBase> SymbolContextBase::resolveSymbolEntry(shared_ptr<Symbol> name) const throw() {
	// resolve aliases
